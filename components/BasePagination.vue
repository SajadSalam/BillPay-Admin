...
<template>
  <div dir="ltr" class="nui-pagination mt-2 !flex !justify-start" :class="[
    rounded && radiuses[rounded],
    color && colors[color],
    props.classes?.wrapper,
  ]">
    <BaseFocusLoop class="nui-pagination-buttons !bg-transparent !border-0" :class="[rounded && radiuses[rounded], props.classes?.buttons]">
      <slot name="before-navigation"></slot>

      <!-- Previous -->
      <NuxtLink :to="paginatedLink(currentPage - 1)" tabindex="0" class="nui-pagination-button !rounded-full !bg-white !border-muted-300 border !w-10"
        :class="props.classes?.button" @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
        @click="(e: any) => handleLinkClick(e, currentPage - 1)">
        <slot name="previous-icon">
          <Icon :name="previousIcon" class="pagination-button-icon !text-primary-600" />
        </slot>
      </NuxtLink>

      <BaseFocusLoop as="ul" class="flex gap-1.5 p-px" :class="[rounded && radiuses[rounded], props.classes?.list]">
        <slot name="before-pagination"></slot>

      <!-- Link -->
      <li>
        <NuxtLink :to="paginatedLink(1)" tabindex="0" class="nui-pagination-link !bg-white !rounded-full " :class="[
      ...
      <li v-for="page in pages" :key="page">
        <NuxtLink :to="paginatedLink(page)" tabindex="0" :aria-current="currentPage === page ? 'page' : undefined"
          class="nui-pagination-link !bg-white !rounded-full" :class="[
      ...
      <li v-if="showLastLink">
        <NuxtLink :to="paginatedLink(lastPage)" tabindex="0" class="nui-pagination-link !bg-white !rounded-full" :class="[
      ...
      <!-- Next -->
      <NuxtLink :to="paginatedLink(currentPage + 1)" tabindex="0" class="nui-pagination-button !border-muted-300 border !w-10 !rounded-full !bg-white"
        :class="props.classes?.button" @keydown.space="(e: any) => (e.target as HTMLAnchorElement).click()"
        @click="(e: any) => handleLinkClick(e, currentPage + 1)">
        <slot name="next-icon">
          <Icon :name="nextIcon" class="pagination-button-icon !text-primary-600" />
        </slot>
      </NuxtLink>

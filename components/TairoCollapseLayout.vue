...
const mainClass = computed(() => {
  ...
  const list = [
    'bg-muted-50 dark:bg-muted-900 relative rounded-s-3xl pb-9 min-h-screen w-full overflow-x-hidden px-4 transition-all duration-300 xl:px-10',
  ]

  if (isOpen.value) {
    list.push('lg:max-w-[calc(100%_-280px)] lg:ms-[280px]')
  }
  else {
...
<template>
  <div class="bg-muted-950 dark:bg-muted-900">
    ...
    <div :class="mainClass">
      <div :class="[
        ...
        <slot name="toolbar">
          <TairoCollapseToolbar v-if="toolbarEnabled" :collapse="props.collapse"
